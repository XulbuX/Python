...
def count_lines(file_path: str) -> int:
  try:
    with open(file_path, 'rb') as f:return sum(1 for _ in f)
  except: return 0
...
def calc_files_scope(files: list) -> tuple[int, int, int]:
  with ThreadPoolExecutor() as executor: results = list(executor.map(process_file, files))
...
def main():
  ...
  if 'scope' not in IGNORE: info += f'  |  FILES SCOPE: {files_scope} lines'
  if 'size' not in IGNORE: info += f'  |  FILES SIZE: {files_size}'
