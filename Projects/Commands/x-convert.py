...
DEFAULT_JSON = {
    'component_replacements': {
        '>> OLD COMPONENT-NAME': ['>> NEW COMPONENT-NAME (`:filename` WILL BE REPLACED WITH THE FILENAME FROM THE IMPORT-PATH)', '>> COMPONENTS IMPORT-PATH'],
        'iconify-icon': 'i',
    ...
    'php_as_js_functions': {
        '>> PHP-FUNCTION NAME': ['>> JS-FUNCTION', ['>> IMPORT UNDER NAME (OPTIONAL IF LIBRARY IS NEEDED IN JS-FUNCTION)', '>> LIBRARY IMPORT-PATH (SEPARATE WITH `\n` IF MULTIPLE NEEDED LIBRARIES - OPTIONAL IF LIBRARY IS NEEDED IN JS-FUNCTION)']],
        'date': ["const date = (str:string) => dayjs(str, 'DD.MM.YYYY').format('DD.MM.YYYY')", ['dayjs', 'dayjs']],
        'dateFormat': ["const dateFormat = (date:string, format:string = 'dd. DD.MM.YYYY') => dayjs(date).format(format);", ['dayjs', 'dayjs']],
...
def get_json(args:dict) -> dict:
    try:
        ...
        if xx.Cmd.confirm(f'      \tCreate [+|b]{JSON_FILE}[*] with default values in program directory?', end=''):
            xx.Json.create(DEFAULT_JSON, indent=4, force=True)
            xx.Cmd.info(f'[white]{JSON_FILE}[*] created successfully.', end='\n')
            xx.FormatCodes.print('        \t[dim]Restarting program...[_]', xx.DEFAULT.text_color)
            main(args)
            xx.Cmd.pause_exit(exit=True)
        else: xx.Cmd.exit()

def get_missing_args(args:list) -> list:
    if not args['filepath']['value'] or not args['blade-vue']['exists']: print()
    if not args['filepath']['value']: args['filepath']['value'] = input(xx.FormatCodes.to_ansi('Path to your file[_|dim] >  [_]', '#3EE6DE')).strip()
    if not args['blade-vue']['exists']:
        ...
        args['blade-vue']['exists'] = input(xx.FormatCodes.to_ansi('                 [_|dim] >  [_]', '#3EE6DE')).strip() == '1'
    return args
    ...
    try:
        if JSON['is_in_env_vars'] != base_dir:
            if not xx.EnvVars.has_path(base_dir=True):
                ...
                xx.FormatCodes.print(f'        \t[#7090FF]If existent, you can execute the program with the command [#FF9E6A]{COMMAND}[#7090FF].[_]', xx.DEFAULT.text_color)
                if xx.Cmd.confirm('      \tAdd the [+|b]program directory[*] to your environment variables?', start='', end=''):
                    xx.EnvVars.add_path(base_dir=True)
                    xx.Cmd.info(f'Successfully added [white]{base_dir}[_] to your environment variables.', end='\n')
                    xx.FormatCodes.print(f'        \t[#7090FF]If the command [#FF9E6A]{COMMAND}[#7090FF] doesn\'t work, you may need to restart the console.[_]', xx.DEFAULT.text_color)
                    xx.FormatCodes.print('        \t[dim]Continuing program...[_]', xx.DEFAULT.text_color)
...
class blade_to_vue:
    ...
    def transform_slots(code:str) -> str|None:
        pattern_oneline = r'<x-slot\s*name\s*=\s*' + QUOTES + r'\s*>\s*\n?\s*([^\n>]+)?\s*\n?\s*</x-slot>'
    ...
    def add_php_funcs(code:str, php_as_js_functions:dict) -> None:
        for php_func in xx.Data.remove_duplicates([func[0] for func in xx.Code.get_func_calls(code)]):
    ...
    def transform_script_lang_funcs(code:str) -> str:
        pattern = r'(<script(?:\s+[\s\S]*)?>)' + xx.Regex.all_except(r'<|>', is_group=True) + r'(<\/script\s*>)'
    ...
    def transform_tag_names(code:str, component_replacements:dict) -> str:
        ...
        def replace_tag_name(match:re.Match) -> str:
            ...
            if tag_parts[1] in component_replacements.keys():
                ...
                if import_path:
                    replacement_name = replacement_name.replace(':filename', pathlib.Path(import_path).stem)
                    if import_path not in JS_IMPORTS: JS_IMPORTS[replacement_name] = import_path
                if tag_parts[2]: tag_parts[2] = rx.sub(r':?([\w-]+)\s*=\s*([\'"])(?:\s*\{\{)?\s*' + L_FN  + r'\s*' + R_BR + r'\s*(?:\}\}\s*)?\2', r'\1="\3"', tag_parts[2])
    ...
    def transform_func_names(code:str, function_replacements:dict) -> str:
        def replace_func_name(match:re.Match) -> str:
            ...
            if func_parts[0] in function_replacements.keys(): return f'{function_replacements[func_parts[0]]}({func_parts[1]})'
            return match.group(0)
        ...
        def replace_loop(match:re.Match) -> str:
            ...
            if len(parts) == 2:
                ...
                if '=>' in item:
                    ...
                    if key.strip() in ['$key', '$index']: key = '$idx'
                    return f'<div v-for="({value.strip()}, {key.strip()}) in {items}" :key="{key.strip()}">'
                else:
                    if item.strip() in ['$key', '$index']: item = '$idx'
    ...
    def transform_concatenated(code:str) -> str:
        ...
        def is_quoted(s:str) -> re.Match: return re.match(r'^[\'"`].*[\'"`]$', s.strip())
        matches = list(rx.compile(concat_code_regex, rx.VERBOSE).finditer(code))
        if matches:
            for match in matches:
                concat_str = match.group(1)
                parts = xx.Data.remove_empty_items(rx.compile(split_concat_regex, rx.VERBOSE).findall(concat_str))
                if any(char in item for item in parts for char in ['"', "'", '`']):
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if is_quoted(part):
                            ...
                            if i == 0 and not is_quoted(parts[i+1]): transformed += f'${{{part} +'
                            elif i == len(parts) - 1 and not is_quoted(parts[i-1]): transformed += f' {part}}}'
                            elif i == 0 or i == len(parts) - 1: transformed += f'${{{part}}}'
                            else:
                                prev_quoted = is_quoted(parts[i-1])
                                next_quoted = is_quoted(parts[i+1])
                                if not prev_quoted and not next_quoted: transformed += f' {part} +'
                                elif prev_quoted and not next_quoted: transformed += f'${{{part} +'
                                elif not prev_quoted and next_quoted: transformed += f' {part}}}'
                                else: transformed += f'${{{part}}}'
                    if not (is_str == len(parts)): transformed = f'`{transformed}`'
                else: transformed = ' + '.join(parts)
                code = code.replace(concat_str, transformed)
        return code

    @staticmethod
    def transform_self_closing_tags(code:str, disallowed_tags:list = [
        'area','base','br','col','embed','hr','img','input','link','meta','param','source','track','wbr'
        ]) -> str:
        def replace_self_closing_tag(match:re.Match) -> str:
            tag = match.group(2)
            if tag in disallowed_tags: return f'<{tag} {match.group(3).strip()}>'
            return match.group(0) if match.group(1) else f'<{tag} {match.group(3).strip()} />'
        code = re.sub(r'<(/?)([\w.-]+)\s*(' + xx.Regex.all_except(r'<|>') + r')\s*/>', replace_self_closing_tag, code, flags=re.DOTALL)  # CORRECT ALREADY PRESENT, BUT FORBIDDEN SELF-CLOSING TAGS
        return re.sub(r'<(/?)([\w.-]+)\s*(' + xx.Regex.all_except(r'<|>') + r')>\s*</\2\s*>', replace_self_closing_tag, code, flags=re.DOTALL)

    @staticmethod
    def update_js(code:str, imports:dict, add_js:list, indent:int = None) -> str:
        if not (imports or add_js): return code
        if not indent or indent < 0: indent = 0
        pattern = r'<script(\s+[\s\S]*)?>' + xx.Regex.all_except(r'<\/script\s*>', is_group=True) + r'<\/script\s*>\s*(?=\s*$)'
        if imports: js_parts = {'new': [f'import {name} from \'{path}\';' for (name), path in imports.items()]}
        if add_js: js_parts['new'] += [''] + add_js
        ...
        if re.search(pattern, code, flags=re.DOTALL): code = re.sub(pattern, add_new_script, code, flags=re.DOTALL)
        else:
    ...
    def convert(self, code:str, indent:int = None) -> str:
        ...
        code = rx.sub(r'(?i)([\w.-]*\s*[)\]}]?)\s*->\s*count\s*' + R_BR, r'\1.length', code)  # REPLACE `value->count()` WITH `value.length`
        code = rx.sub(r'(?i)(?:count|strlen)\s*' + R_BR, r'\1.length', code)  # REPLACE `count(  )` AND `strlen(  )` WITH `.length`
        code = rx.sub(r'<([\w.-]+)\s+\{\{\s*(\$[\w_]+)\s*->\s*merge\s*\(\s*' + S_BR + r'\s*\)\s*\}\}(.*?)/?>(\s*(?:\n|<[\w.-]+\s+|$))',
                      ...
                      code)  # TRANSFORM `<tagname {{ $var.merge(['key_name' => 'values']) }} ...>` INTO `<tagname key_name="values" ...>`
        code = re.sub(r'([\w.-]*\s*[)\]}]?)\s*->\s*(?=\w\s*(\(.*?\))?)', r'\1.', code)  # TRANSFORM ARROW CHAINS TO DOT CHAINS
        ...
        code = re.sub(r'\$(key|index)(?!\s*\()', '$idx', code)  # CHANGE VARIABLES `$key`, `$index` TO `$idx`
        code = re.sub(r'\s*loop\.first\s*', 'idx == 0', code)  # CHANGE `loop.first` TO `idx == 0`
        code = re.sub(r'\s*loop\.last\s*', 'idx == ….length - 1', code)  # CHANGE `loop.last` TO `idx == ….length - 1`
        code = re.sub(r'(?<=[^!<>=\s])\s*([!=])=\s*(?=[^=\s])', r' \1== ', code)  # CORRECT EQUALS
        code = re.sub(r'(?<![\w$])(\s*:?([\w-]+)\s*=\s*([\'"]))\3', '', code)  # REMOVE EMPTY ATTRIBUTES
        code = rx.sub(r'\s*:\s*init-errors\s*=\s*' + QUOTES + r'(\s*(?:\n|$))', '', code)  # REMOVE `init-errors` VARIABLES
        code = re.sub(r'(?:<div\s*>\s*)?{{\s*\$(slot)\s*}}(?:\s*</div\s*>)?', r'<\1 />', code)  # REPLACE `{{ $slot }}` WITH `<slot />`
        code = re.sub(r'(?i)(@php)', r'<!-- \1', code)  # REPLACE `@php` SYNTAX WITH `<!--` COMMENT-START
        code = re.sub(r'(?i)\s*@endphp', r' -->', code)  # REPLACE `@endphp` SYNTAX WITH `-->` COMMENT-END
        code = rx.sub(r'import\s*{\s*route\s*}\s*from\s*' + QUOTES + r'\s*;', "import { route } from '@/plugins/route';", code)  # REPLACE `import { route } from '...';` WITH `import { route } from '@/plugins/route';`
        code = rx.sub(r'(?i)(<(?!/)[\w.-]+' + xx.Regex.all_except(r'<|>|@if') + r')@if\s*' + R_BR + r'((?:\s+[\w-]+)?)(' + xx.Regex.all_except(r'<|>') + r'>)', lambda m: f'{m.group(1)}:{m.group(3).strip()}="{m.group(2).strip()}"{m.group(4)}' if m.group(3).strip() else m.group(0), code, flags=re.DOTALL)  # REPLACE `<div @if(true) attr>` SYNTAX WITH `<div :attr="true">` ATTRIBUTES
        code = rx.sub(r'(?i)(<(?!/)[\w.-]+' + xx.Regex.all_except(r'<|>|@empty') + r')@empty\s*' + R_BR + r'((?:\s+[\w-]+)?)(' + xx.Regex.all_except(r'<|>') + r'>)', lambda m: f'{m.group(1)}:{m.group(3).strip()}="!({m.group(2).strip()})"{m.group(4)}' if m.group(3).strip() else m.group(0), code, flags=re.DOTALL)  # REPLACE `<div @empty(var) attr>` SYNTAX WITH `<div :attr="!var">` ATTRIBUTES
        code = rx.sub(r'(?i)(<(?!/)[\w.-]+' + xx.Regex.all_except(r'<|>|@isset') + r')@isset\s*' + R_BR + r'((?:\s+[\w-]+)?)(' + xx.Regex.all_except(r'<|>') + r'>)', lambda m: f'{m.group(1)}:{m.group(3).strip()}="({m.group(2).strip()}) !== undefined"{m.group(4)}' if m.group(3).strip() else m.group(0), code, flags=re.DOTALL)  # REPLACE `<div @isset(var) attr>` SYNTAX WITH `<div :attr="(var) !== undefined">` ATTRIBUTES
        code = rx.sub(r'(?i)(<(?!/)[\w.-]+' + xx.Regex.all_except(r'<|>|@is_null') + r')@is_null\s*' + R_BR + r'((?:\s+[\w-]+)?)(' + xx.Regex.all_except(r'<|>') + r'>)', lambda m: f'{m.group(1)}:{m.group(3).strip()}="({m.group(2).strip()}) === null"{m.group(4)}' if m.group(3).strip() else m.group(0), code, flags=re.DOTALL)  # REPLACE `<div @is_null(var) attr>` SYNTAX WITH `<div :attr="(var) === null">` ATTRIBUTES
        code = rx.sub(r'(?i)(<(?!/)[\w.-]+' + xx.Regex.all_except(r'<|>|@end(?:[\w_]+)') + r')@end([\w_]+)(' + xx.Regex.all_except(r'<|>') + r'>)', r'\1\3', code, flags=re.DOTALL)  # REMOVE `@end...` SYNTAX INSIDE TAG-ATTRIBUTES
        code = rx.sub(r'(?i)@if\s*' + R_BR, r'<span v-if="\1">', code)  # REPLACE `@if(  )` SYNTAX WITH `<span v-if="  ">` TAGS
        code = rx.sub(r'(?i)@empty\s*' + R_BR, r'<span v-if="!(\1)">', code)  # REPLACE `@empty(  )` SYNTAX WITH `<span v-if="!(  )">` TAGS
        code = rx.sub(r'(?i)@isset\s*' + R_BR, r'<span v-if="(\1) !== undefined">', code)  # REPLACE `@isset(  )` SYNTAX WITH `<span v-if="(  ) !== undefined">` TAGS
        code = rx.sub(r'(?i)@is_null\s*' + R_BR, r'<span v-if="(\1) === null">', code)  # REPLACE `@is_null(  )` SYNTAX WITH `<span v-if="(  ) === null">` TAGS
        code = rx.sub(r'(?i)@elseif\s*' + R_BR, r'</span><span v-else-if="\1">', code)  # REPLACE `@elseif(  )` SYNTAX WITH `</span><span v-else-if="  ">` TAGS
        code = re.sub(r'(?i)@else(\s+)', r'</span><span v-else>\1', code)  # REPLACE `@else` SYNTAX WITH `</span><span v-else>` TAGS
        code = re.sub(r'(?i)@end([\w_]+)', lambda m: '</span>' if m.group(1) in ['if','isset','elseif','else'] else '</div>', code)  # REPLACE `@end...` SYNTAX WITH `</div>` or `</span>` END-TAGS
        code = rx.sub(r'<([\w-]+)\s+v-if\s*=\s*"\s*\$?slot\s*"\s*>\s*<slot(?:\s+name\s*=\s*' + QUOTES + r')?\s*>\s*{{\s*\$?slot\s*}}\s*</slot\s*>\s*</\1\s*>', lambda m: f'<slot{f' name="{m.group(3).strip()}"' if m.group(3).strip() else ''} />', code)  # REPLACE `<div v-if="$slot"><slot name="...">{{ $slot }}</slot></div>` WITH `<slot />` OR `<slot name="..." />`
        code = rx.sub(L_FN + r'\s*\((.*?\))', r'$t(\1', code)  # REPLACE `__()` AND `$lang()` FUNCTIONS WITH `$t()` FUNCTION
        code = re.sub(r'{!!\s*(.*?)\s*!!}', r'<div v-html="\1"/>', code)  # REPLACE `{!!  !!}` SYNTAX WITH `<div v-html="  "/>` TAGS
        code = re.sub(r':([\w-]+)\s*=\s*([\'"])\s*\$?\1\s*\2', r':\1', code)  # REPLACE `:input="$input"` WITH `:input`
        code = rx.sub(r':?([\w-]+)\s*=\s*' + QUOTES, lambda m: f':{m.group(1)}="{m.group(3).strip()}"' if xx.Code.is_js(m.group(3).strip()) else f'{m.group(1)}="{m.group(3).strip()}"', code)  # REPLACE LEFTOVER `attr="JS"` WITH `:attr="JS"` OR `:attr="string"` WITH `attr="string"`
        code = rx.sub(r':?([\w-]+)\s*=\s*([\'"])\s*\{' + C_BR + r'\}\s*\2', lambda m: f':{m.group(1)}="{m.group(3).strip()}"', code)  # REPLACE LEFTOVER `attr="{{  }}"` WITH `:attr="  "`
        code = rx.sub(r':?([\w-]+)\s*=\s*([\'"])(((?:\\.|(?:(?!\2).)+)*)\{' + C_BR + r'\}((?:\\.|(?:(?!\2).)+)*))\2', lambda m: f':{m.group(1)}="`{m.group(3).replace('{{', '${').replace('}}', '}')}`"', code)  # REPLACE `{{  }}`-CONCATENATED STRINGS WITH BACKTICK-STRINGS
        code = rx.sub(r'(?i)asset\s*' + R_BR, r'\1', code)  # REMOVE `asset(  )` BRACKETS
        code = rx.sub(r'(?i)isset\s*' + R_BR, r'(\1)', code)  # REMOVE `isset(  )` BRACKETS
        code = re.sub(r'(\n\s*)(@[\w_]+.*?)(\s*\n)', r'\1<!-- \2 -->\3', code)  # COMMENT OUT LEFTOVER BLADE SYNTAX
        code = rx.sub(r':v-([\w-]+)\s*=\s*' + QUOTES, r'v-\1="\3"', code)  # REMOVE `:` FROM VUE-ATTRIBUTES
        code = re.sub(r':?([\w-]+)\s*=\s*([\'"])\s*((?:\\\2|(?:(?!\2)[\'"]))+)((?:\\.|(?:(?!\2).)+)*)\3\s*\2', lambda m: f'{m.group(1)}="{m.group(4).strip().replace("\\'", "'")}"', code)  # REMOVE UNNECESSARY QUOTATION MARKS
        code = rx.sub(r'\$attributes\s*\[\s*' + QUOTES + r'\s*\]', r'\2', code)  # REMOVE UNNECESSARY VARIABLE INPUTS
        code = re.sub(r'\$(\w+)(?!\s*\()', lambda m: m.group(1) if not m.group(1) == 't' else m.group(0), code)  # REMOVE LEFTOVER `$` PREFIXES FROM VARS AND FUNCTIONS

        outside_template_script_pattern = r'(<script(?:\s+[\s\S]*)?>' + xx.Regex.all_except(r'<\/script\s*>') + r'<\/script\s*>\s*)(?=\s*$)'
        outside_template_script = '\n'.join(xx.String.multi_rstrip(rx.findall(outside_template_script_pattern, code, flags=re.DOTALL), ' \n'))
        code = rx.sub(outside_template_script_pattern, '', code, flags=re.DOTALL)  # REMOVE OUTSIDE TEMPLATE SCRIPT
...
def main(args:dict):
    ...
    if converted_content:
        ...
        if os.path.exists(new_file_path):
            ...
            if xx.Cmd.confirm(f'      \tDo you want to replace [+|b]{os.path.basename(new_file_path)}[*]?', end=''):
                pass
            else:
...
if __name__ == "__main__":
    ...
    if DEBUG:
        if args['help']['exists']: show_help()
        else: main(args)
    else:
        try:
            if args['help']['exists']: show_help()
            else: main(args)
        except FileNotFoundError: xx.Cmd.fail(f'File not found: [white]{args['filepath']['value']}', pause=DEBUG)
        except KeyboardInterrupt: xx.Cmd.exit(start='\n\n')
        except Exception as e:    xx.Cmd.fail(e, pause=DEBUG)
