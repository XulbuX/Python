...
*   **Naming Conventions:**
    *   **Descriptive Variable Names:** Single-letter variables (e.g., `c`, `r`, `f`) are strictly banned. The ONLY exceptions are `i` (and rarely `j`) for loop indices, `n` for mathematical counts/parameters, and `x`, `y`, `z` for spatial/cartesian coordinates. Always use fully descriptive variable names (e.g., `ch` or `channel`, `red`, `modifier`).
